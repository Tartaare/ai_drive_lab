...
const quaternionToRotation = (callback: (v: Triplet) => void) => {
  return (v: Quad) =>
    callback(e.setFromQuaternion(q.fromArray(v)).toArray() as Triplet);
};
  ...
  useLayoutEffect(() => {
    ...
    const objectCount =
      object instanceof InstancedMesh
        ? (object.instanceMatrix.setUsage(DynamicDrawUsage), object.count)
        : 1;
  ...
  const api = useMemo<RaycastVehiclePublicApi>(() => {
    return {
      ...
      setBrake(brake: number, wheelIndex: number) {
        const uuid = getUUID(ref);
        uuid &&
          worker.setRaycastVehicleBrake({ props: [brake, wheelIndex], uuid });
      },
