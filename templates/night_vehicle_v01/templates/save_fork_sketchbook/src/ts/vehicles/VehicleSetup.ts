...
export class VehicleSetup
{
	public static prepareModel(model: THREE.Object3D): void
	{
		...
		const attachmentQueue: Array<{wheel: any, sibling: any}> = [];
		...
		model.traverse((child: any) => {
			if (child.isMesh || child.isGroup || child.type === 'Object3D')
			{
				...
				if (name.includes('wheel'))
				{
					...
					if (isWheel)
					{
						// Attempt to find associated parts (Rims, Hubcaps, Brakes) and attach them to the wheel
						// This fixes the issue where rims don't rotate with the tire
						const parent = child.parent;
						if (parent) {
							const siblings = parent.children.filter(c => c !== child && c.type !== 'Bone');
							const sideIdentifiers = [];
							if (name.includes('fl') || (name.includes('front') && name.includes('left'))) sideIdentifiers.push('fl', 'front_left', 'frontleft');
							else if (name.includes('fr') || (name.includes('front') && name.includes('right'))) sideIdentifiers.push('fr', 'front_right', 'frontright');
							else if (name.includes('rl') || (name.includes('rear') && name.includes('left'))) sideIdentifiers.push('rl', 'rear_left', 'rearleft');
							else if (name.includes('rr') || (name.includes('rear') && name.includes('right'))) sideIdentifiers.push('rr', 'rear_right', 'rearright');

							siblings.forEach(sibling => {
								const siblingName = sibling.name.toLowerCase();
								// If sibling matches the same side and looks like a wheel part
								if (sideIdentifiers.some(id => siblingName.includes(id)) && 
									(siblingName.includes('rim') || siblingName.includes('hubcap') || siblingName.includes('brake') || siblingName.includes('disc'))) {
									
									// Queue attachment for after traverse
									attachmentQueue.push({ wheel: child, sibling: sibling });
									console.log(`Queued attachment: ${sibling.name} to ${child.name}`);
								}
							});
						}
		...
		// Process attachment queue AFTER traverse is complete
		attachmentQueue.forEach(item => {
			try {
				item.wheel.attach(item.sibling);
				console.log(`Auto-attached ${item.sibling.name} to ${item.wheel.name}`);
			} catch (e) {
				console.warn(`Failed to attach ${item.sibling.name}:`, e);
			}
		});
